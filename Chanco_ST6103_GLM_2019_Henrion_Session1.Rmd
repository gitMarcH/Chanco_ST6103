...
Prove that $\hat{\beta}_0$ and $\hat{\beta_1}$ are the OLS solution.
...
$\text{ }$ $\;\;\;\Rightarrow \sum_i y_i -n\hat{\beta_0}-\hat{\beta}_1\sum_ix_i = 0$
...
* $\hat{\beta}_0$ is the estimated *intercept* of the fitted regression line
* $\hat{\beta}_1$ is the estimated *slope* of the fitted regression line
...
$R^2$ is also the squared sample correlation coefficient.
...
E.g. $Y=\beta_0+\beta_1X+\beta_2X^2+\epsilon$ is a general linear model but $Y=\beta_0+\beta_1X+\beta_1^2X^2+\epsilon$ is not.
