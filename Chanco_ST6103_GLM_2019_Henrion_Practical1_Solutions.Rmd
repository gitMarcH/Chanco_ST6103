...
c. $R^2=\rho(\mathbf{x},\mathbf{y})^2$ (in the case of a single predictor variable $X$)


# Exercise 1

![Exercise 1a solution](ST6103_Pract1_Ex1a.jpg)

# Exercise 1

![Exercise 1b solution](ST6103_Pract1_Ex1b.jpg)

# Exercise 1

![Exercise 1c solution](ST6103_Pract1_Ex1c.jpg)
...
![Exercise 2a & 2d solution](LinearRegressionByHand_Pract1Ex2_Sol.png)

# Exercise 2

2b. Relationship between the 2 variables.

  * Negative relationship between $X$ and $Y$. Linear model seems (visually) reasonable.
  
  * For every unit increase in $X$, $Y$ sdecreases, on average, but 0.49.
  
  * When $x=0$, $\hat{y}=10.06$

```{r}
xx<-seq(-5,8,length=1000)
yy<-10.0650-0.4878*xx
...
2c. Visually, yes. $R^2$=`r yhat<-10.065-0.4878*x; round(digits=2,sum((yhat-mean(y))^2)/sum((y-mean(y))^2))`.
As for this linear regression model we do not make any assumptions about the distribution of the residuals $\epsilon_i$, we do not need to check them for normality etc.

2e. Prediction

```{r}
xNew<-c(-40.1,1,2.5,100)
yHat<-10.0650-0.4878*xNew

print(rbind(xNew,yHat))
```
...
![Exercise 3 solution](ST6103_Pract1_Ex3.jpg)
...
![Exercise 4 solution](ST6103_Pract1_Ex4.jpg)
